...
const AppLayout = ({ children }) => {
  ...
  const NavItem = ({ item, onClick, isSecondary = false }) => {
    ...
    return (
      <Link
        ...
        className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors font-medium
          ${active 
            ? 'bg-[#E03131]/10 text-[#E03131] dark:bg-[#E03131]/20 dark:text-[#F03E3E]' 
            : 'text-slate-900 dark:text-white hover:text-[#E03131] dark:hover:text-[#F03E3E] hover:bg-slate-100 dark:hover:bg-slate-800/50'
          }
        `}
      >
  ...
  return (
    <div className="flex h-screen bg-app">
      {/* Sidebar */}
      <aside className="w-[280px] h-full bg-white dark:bg-[#0F141B] border-r border-slate-200 dark:border-slate-800/60 backdrop-blur flex flex-col sticky top-0">
        {/* Brand Lockup */}
        <Link 
          to="/" 
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          ...
          <div>
            <button
              ...
              className="w-full flex items-center justify-between gap-3 px-3 py-2 rounded-lg transition-colors text-slate-900 dark:text-white font-medium hover:text-[#E03131] dark:hover:text-[#F03E3E] hover:bg-slate-100 dark:hover:bg-slate-800/50"
            ...
            {isMoreOpen && (
              <div className="mt-1 ml-3 space-y-2 pl-2 border-l-2 border-slate-800/60">
                {moreItems.map((item, index) => (
                  <NavItem key={item.path || index} item={item} isSecondary={true} />
                ))}
                ...
                <div>
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-slate-900 dark:text-white font-medium hover:text-[#E03131] dark:hover:text-[#F03E3E] hover:bg-slate-100 dark:hover:bg-slate-800/50"
